...
<head>
    ...
    <title>Recent Flow Readings - AquWatch</title>
    ...
    <div class="mx-auto max-w-6xl space-y-5">
        <div class="glass-card rounded-3xl p-5 md:p-7 shadow-xl">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h1 class="text-3xl font-extrabold text-cyan-950">Recent Flow Readings</h1>
                    <p class="text-cyan-900/80 mt-1">Optional detailed table for advanced users</p>
                </div>
                <div class="flex gap-2">
                    <a href="{{ route('contents.flow-display') }}" class="px-4 py-2 rounded-xl bg-cyan-700 text-white hover:bg-cyan-800 transition">Back to Flow Display</a>
                    <a href="{{ route('contents.graph-display') }}" class="px-4 py-2 rounded-xl bg-white/70 text-cyan-900 hover:bg-white transition">Open Graph Display</a>
                </div>
            </div>
        </div>
            ...
            <table class="w-full text-left text-sm">
                <thead class="text-cyan-900/80 uppercase text-xs">
                    <tr>
                        ...
                        <th class="py-2 pr-4">Total (mL)</th>
                    </tr>
                </thead>
                <tbody class="text-cyan-950">
                    @forelse ($readings as $reading)
                        <tr class="border-t border-cyan-900/10">
                            ...
                            <td class="py-2 pr-4">{{ number_format((float) $reading->flow_lpm, 3) }}</td>
                            <td class="py-2 pr-4">{{ number_format((int) $reading->total_ml) }}</td>
                        </tr>
                    @empty
                        <tr>
