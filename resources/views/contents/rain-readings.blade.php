...
<head>
    ...
    <title>Recent Rain Readings - AquWatch</title>
    ...
    <div class="mx-auto max-w-6xl space-y-5">
        <div class="glass-card rounded-3xl p-5 md:p-7 shadow-xl">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h1 class="text-3xl font-extrabold text-cyan-950">Recent Rain Readings</h1>
                    <p class="text-cyan-900/80 mt-1">Optional detailed table for advanced users</p>
                </div>
                <div class="flex gap-2">
                    <a href="{{ route('contents.rain-display') }}" class="px-4 py-2 rounded-xl bg-cyan-700 text-white hover:bg-cyan-800 transition">Back to Rain Display</a>
                    <a href="{{ route('contents.rain-graph-display') }}" class="px-4 py-2 rounded-xl bg-white/70 text-cyan-900 hover:bg-white transition">Open Graph Display</a>
                </div>
            </div>
        </div>
            ...
            <table class="w-full text-left text-sm">
                <thead class="text-cyan-900/80 uppercase text-xs">
                    <tr>
                        ...
                        <th class="py-2 pr-4">Analog Value</th>
                ...
                <tbody class="text-cyan-950">
                    @forelse ($readings as $reading)
                        <tr class="border-t border-cyan-900/10">
                            ...
                            <td class="py-2 pr-4">{{ number_format((int) $reading->analog_value) }}</td>
                            <td class="py-2 pr-4">{{ str_replace('_', ' ', ucfirst($reading->intensity_level)) }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="py-3 text-cyan-900">No readings available yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
